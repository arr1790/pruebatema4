...
async function eliminarPaciente(formData) {
    ...
    revalidatePath("/pacientes-db") //para que se actualice la pagina
}
...
async function insertarPaciente(formData) {
    ...
    await connection.query("INSERT INTO pacientes (nombre, localidad, fecha_nacimiento) VALUES (?,?,?)", [nombre, localidad, fecha_nacimiento])
    ...
    return (
        ...
        <>
            ...
            <div className="max-w-4xl mx-auto mt-10 p-5 bg-gray-100 shadow-md rounded-lg">
                <h2 className="text-xl font-semibold text-gray-800 mb-4">Lista de Pacientes de base de datos</h2>
                        ...
                        rows.map((paciente) => (
                            <div key={paciente.id} className="p-4 bg-white shadow-sm rounded-md flex justify-between items-center">
                                ...
                                <div className="flex space-x-2">
                                    <Link href={`/pacientes-db/${paciente.id}`} className="text-blue-500 hover:underline">Ver</Link>
                                    <Link href={`/pacientes-db/${paciente.id}/modificar`} className="text-yellow-500 hover:underline">Modificar</Link>
